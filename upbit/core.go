package upbit

import (
	"fmt"
	"strings"
	"sync"
	"time"

	"github.com/google/uuid"
	"github.com/neosouler7/bookstore-go/commons"
	"github.com/neosouler7/bookstore-go/exchange"
	"github.com/neosouler7/bookstore-go/websocketmanager"
)

const name = "upb"

type Upbit struct{}

func (u *Upbit) Ping() {
	websocketmanager.Ping(name)
}

func (u *Upbit) Subscribe(pairs []string, wg *sync.WaitGroup) {
	defer wg.Done()
	time.Sleep(time.Second * 1)

	var streamSlice []string
	for _, pair := range pairs {
		pairInfo := strings.Split(pair, ":")
		market, symbol := strings.ToUpper(pairInfo[0]), strings.ToUpper(pairInfo[1])
		streamSlice = append(streamSlice, fmt.Sprintf("'%s-%s'", market, symbol))
	}

	uuid := uuid.NewString()
	streams := strings.Join(streamSlice, ",")
	msg := fmt.Sprintf("[{'ticket':'%s'}, {'type': 'orderbook', 'codes': [%s]}]", uuid, streams)

	websocketmanager.SendMsg(name, msg)
	fmt.Printf(websocketmanager.SubscribeMsg, name)
}

func (u *Upbit) HandleWsMessage(msgBytes []byte) {
	if strings.Contains(string(msgBytes), "status") {
		fmt.Println("PONG") // {"status":"UP"}
		return
	}
	var rJson interface{}
	commons.Bytes2Json(msgBytes, &rJson)
	SetOrderbook("W", name, rJson.(map[string]interface{}))
}

func (u *Upbit) HandleRestResponse(rJson map[string]interface{}) {
	SetOrderbook("R", name, rJson)
}

func Run(e string) {
	exchange.Run(e, &Upbit{})
}
